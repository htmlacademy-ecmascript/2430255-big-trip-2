...
function createPointTemplate(point, offers, destinations) {
  ...
  const destinationName = pointDestination
    ? pointDestination.name
    : destination;

  const selectedOffers = pointOffers
    .map(
      (offer) =>
  ...
  return `
    <li class="trip-events__item">
      <div class="event">
        <time class="event__date" datetime="${convertDate(
    dateFrom,
    'FULL_DATE'
  )}">${convertDate(dateFrom, 'SHORT_DATE')}</time>
        ...
        <div class="event__schedule">
          <p class="event__time">
            <time class="event__start-time" datetime="${convertDate(
    dateFrom,
    'FULL_DATE_AND_TIME'
  )}">${convertDate(dateFrom, 'ONLY_TIME')}</time>
            &mdash;
            <time class="event__end-time" datetime="${convertDate(
    dateTo,
    'FULL_DATE_AND_TIME'
  )}">${convertDate(dateTo, 'ONLY_TIME')}</time>
          </p>
